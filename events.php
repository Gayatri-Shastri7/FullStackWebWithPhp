...
<body>
	...
	<div class="section">
		<div class="content-wrap">
			...
			<div class="container">
				<h2 class="title upcomming_text">Upcoming Events </h2>
				<div class="row" id="upcommingEvents">
					<!-- get data event data form databse  -->
					<?php
					$today =  date('Y-m-d');
					$get_all_data = $db->query("SELECT * FROM events WHERE start_date >= '$today' ORDER BY start_date,start_time ASC");
					if ($get_all_data->num_rows != 0) {
						...
						<div class="box-fundraising">
							<div class="media">
								...
								<img src="ad/admin/' . $data["thumb"] . '" alt="' . $data["name"] . '">
							</div>
							<div class="body-content">
								<p class="title"><a target="_blank" href="events-single.php?event_id=' . $data["event_id"] . '">' . $data["name"] . '</a></p>
								<div class="meta">
									<span class="date"><i class="fa fa-clock-o"></i> ' . date_format($satrt_time, "h:i:A")  . ' - ' . date_format($end_time, "h:i:A") . '</span>
									<span class="location"><i class="fa fa-map-marker"></i>  ' . $data['location'] . '</span>
								</div>
								<div class="text">' . $data["description"] . '</div>
								<div class="spacer-30"></div>
								<a  target="_blank" href="events-single.php?event_id=' . $data['event_id'] . '" class="btn btn-primary">READ MORE</a>
							</div>
						</div>
					</div>';
						}
					} else {
						echo '<style> .upcomming_text{
display:none !important;
				...
				<h2 class="title  my-4">Past Events </h2>
				<div class="row" id="pastEvents">
					<!-- get end_data event end_data form end_databse  -->
					<?php
					$today =  date('Y-m-d');
					$get_all_end_data = $db->query("SELECT * FROM events WHERE end_date <'$today' ORDER BY start_date,start_time ASC");
					if ($get_all_end_data->num_rows != 0) {
						while ($end_data = $get_all_end_data->fetch_assoc()) {
							$start_date = date_create($end_data["start_date"]);
							$satrt_time = date_create($end_data["start_time"]);
							$end_time = date_create($end_data["end_time"]);
							echo '	<div class="col-sm-4 col-md-4">
						<div class="box-fundraising">
							<div class="media">
								<div class="meta-date">
								...
								<img src="ad/admin/' . $end_data["thumb"] . '" alt="' . $end_data["name"] . '">
							</div>
							<div class="body-content">
								<p class="title"><a target="_blank" href="events-single.php?event_id=' . $end_data["event_id"] . '">' . $end_data["name"] . '</a></p>
								<div class="meta">
									<span class="date"><i class="fa fa-clock-o"></i> ' . date_format($satrt_time, "h:i:A")  . ' - ' . date_format($end_time, "h:i:A") . '</span>
									<span class="location"><i class="fa fa-map-marker"></i>  ' . $end_data['location'] . '</span>
								</div>
								<div class="text">' . $end_data["description"] . '</div>
								<div class="spacer-30"></div>
								<a  target="_blank" href="event_gallery.php?event_id=' . $end_data['event_id'] . '" class="btn btn-primary">View photos</a>
							</div>
						</div>
					</div>';
						}
