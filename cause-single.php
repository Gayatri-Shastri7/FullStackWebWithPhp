...
include_once("includes/causes.php");

$cause = new Cause;
if(isset($_GET['cause_id'])){
  $id = $_GET['cause_id'];
  $causes = $cause->fetch_data($id);
  $cause_id=$causes['cause_id'];
  $cause_name=$causes['cause_name'];
  $cause_main=$causes['cause_maintitle'];
  $cause_content=$causes['cause_context'];
  $cause_goal=$causes['goal'];
  $cause_img=$causes['cause_img'];
} 
?>
			...
			<div class="d-flex justify-content-center bd-highlight mb-3">
			    <nav aria-label="breadcrumb">
				  <ol class="breadcrumb ">
				    ...
				    <li class="breadcrumb-item active" aria-current="page">Cause Detail</li>
				  </ol>
				</nav>
		  	</div>
		</div>
			...
			<div class="container">
				<div class="row">
					<div class="col-sm-12 col-md-12">
						...
						<h2 class="color-secondary"><span class="color-primary"><?php echo $cause_name?></h2>
					</div>

					<div class="col-sm-6 col-md-6">

						<p class="uk18 color-secondary"> <?php echo $cause_main?> </p>
						
						<p><?php echo $cause_content?> </p>

					</div>

					<div class="col-sm-6 col-md-6">
						<div class="img-video">
							<img src="ad/admin/donationimages/<?php echo  $cause_img?> "  alt="">
						</div>
							
							<div class="spacer-30"></div>
							<?php 
													$sql1="SELECT SUM(amount) as total_contributed FROM donations WHERE purpose LIKE '%{$cause_name}%'";
							...
							<div class="progress-fundraising progress-lg">
		              			<div class="total">Donated</div>
		          			  	<div class="persen"><?php 
														echo round(($total_contributed/$cause_goal)*100);
														?>%</div>
		              			<div class="progress">
								  <div class="progress-bar" role="progressbar" aria-valuenow="80" aria-valuemin="0" aria-valuemax="100"></div>
								</div>
								<div class="detail">
									<h3>Rs <?php echo $total_contributed;?><small>&nbsp;Raised of&nbsp;</small>Rs &nbsp;<?php echo $cause_goal;?> <small>Goal</small></h3>
								</div>
							</div>

							<a href="donate.php?cause_id=<?php
													echo $cause_id;
